...
export const ChampionDetail = ({ championSelected, setChampionSelected }) => {
  ...
  useEffect(() => {
    const handleKeyDown = (event) => {
      if (event.key === "d") {
        const selectedHeroIndex = championSummary.findIndex(
          (hero) => hero.name === championSelected.name,
        );
  ...
  return (
    <>
      {selectedHeroDetail && selectedHeroDetail.name !== "None" && (
        <div
          className="w-full min-h-[600px] h-4/5 bg-no-repeat bg-cover"
          style={{
            backgroundImage: `url(${cdBasePath}${selectedHeroDetail.skins[0].uncenteredSplashPath})`,
          }}
        >
          <div className="h-full flex flex-col justify-center gap-2 w-3/6">
            ...
            <p className="ml-4 text-yellow-100 font-serif tracking-wider">
              {selectedHeroDetail.title}
            </p>
            <ChampionRoles roles={selectedHeroDetail.roles} />
            <ChampionPlaystyleInfo
              playstyleInfo={selectedHeroDetail.playstyleInfo}
            />
            <ChampionSkills
              skills={selectedHeroDetail.spells}
              selectedHeroState={selectedHeroDetail}
            />
          </div>
        </div>
      )}
