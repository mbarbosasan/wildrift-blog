  ...
  return (
    <>
      <div className="ml-4 flex items-center gap-2">
        {skills &&
          skills.map((skill, index) => (
            <img
              key={index}
              src={`${basePath}${skill.abilityIconPath.toLowerCase()}`}
              alt={skill.name}
              width={50}
              height={50}
      ...
      {selectedSkill && (
        ...
        >
          <CardHeader>
            <CardTitle className={"text-yellow-200 p-2 text-4xl text-center"}>
              {selectedSkill.name}
            </CardTitle>
          </CardHeader>
          <CardContent className={"flex flex-wrap-reverse justify-between"}>
            ...
            <section
              className={
                "sm:w-full xl:w-2/4 md:min-w-[400px] sm:w-full sm:min-w-[400px]"
              }
            >
              <video
                src={`${basePathAbilityPreview}${selectedSkill.abilityVideoPath}`}
                muted
                controls={true}
                autoPlay
                loop={true}
